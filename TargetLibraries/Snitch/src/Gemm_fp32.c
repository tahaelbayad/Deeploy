...
void gemm_fp32_opt(uint32_t M, uint32_t N, uint32_t K, float32_t *A,
                   ...
                   uint32_t ldC, float32_t *Y, const uint32_t *BETA,
                   uint32_t setup_SSR) {
  ...
  for (uint32_t m = 0; m < M; m++) {
    ...
    snrt_ssr_disable();
    for (; n < N; n++) {
      float32_t c = C[C_offset + m * ldC + n];
      for (uint32_t k = 0; k < K; k++) {
        c += A[A_offset + k + m * ldA] * B[k + n * K];
      }
...
void gemm_fp32(uint32_t M, uint32_t N, uint32_t K, float32_t *A, uint32_t ldA,
  ...
  for (uint32_t m = 0; m < M; m++) {
    for (uint32_t n = 0; n < N; n++) {
      float32_t c0 = C[C_offset + m * ldC + n];
      for (uint32_t k = 0; k < K; k++) {
        c0 += A[A_offset + k + m * ldA] * B[k + n * N];
      }
      Y[C_offset + m * ldC + n] = c0;
    }
  }
}
